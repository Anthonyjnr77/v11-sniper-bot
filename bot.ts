...
// NEW (#2): minimum SOL your trading wallet should hold before you get warned
...
// NEW (#6): one-time startup warning if fees look heavy relative to position size
function logFeeSizingWarning() {
  ...
  console.log(
    `💸 Fee sizing check: Jito tip + priority fee ≈ ${(totalFeeLamports / 1e9).toFixed(4)} SOL ` +
    `(~${pctOfBuy.toFixed(1)}% of your ${(BUY_AMOUNT / 1e9).toFixed(4)} SOL buy size)`
  );
  if (pctOfBuy > 20) {
    console.log("⚠️ Fees are a large fraction of your position size — consider whether the plain RPC path alone is landing often enough that the Jito tip isn't pure overhead.");
  }
}
...
/* ================= WALLET BALANCE CHECK (#2) ================= */

let lastLowBalanceAlertAt = 0;
const LOW_BALANCE_ALERT_COOLDOWN_MS = 30 * 60 * 1000; // don't spam more than once per 30 min
  ...
  try {
    ...
    if (sol < MIN_WALLET_BALANCE_SOL) {
      ...
      if (now - lastLowBalanceAlertAt > LOW_BALANCE_ALERT_COOLDOWN_MS) {
        ...
        console.log(`⚠️ Wallet balance low: ${sol.toFixed(4)} SOL (threshold ${MIN_WALLET_BALANCE_SOL} SOL)`);
        await sendTelegramAlert(
          `⚠️ <b>LOW WALLET BALANCE</b>\nCurrent: ${sol.toFixed(4)} SOL\nThreshold: ${MIN_WALLET_BALANCE_SOL} SOL\nBuys may start failing due to insufficient funds for gas/tips.`
        );
      }
    }
  } catch (e: any) {
...
interface Position {
  ...
  highestValue: number;
  tiersHit: boolean[];
  entryMarketCapUSD: number | null; // NEW — captured at buy time for journal/PnL reference
}
